var gameObject = {
	...
	heroesArray: ["GREEN LANTERN", "BATMAN", "FLASH", "SUPERMAN", "WONDER WOMAN", "HAWKGIRL", "MARTIAN MANHUNTER"],
	randomWord: "",
	heroesLetters:[],
	...
	generateWord: function(){
		...
		this.randomWord = this.heroesArray[random_num];
		this.heroesLetters = this.randomWord.split("");

		console.log(this.randomWord + " " + this.heroesLetters);
	...
	checkMatch: function(){
		...
		for (var i=0; i < this.heroesLetters.length; i++){
			if (this.currentLetter == this.heroesLetters[i]){
				matchCounter++;
			}
		}
	...
	revealHero: function(){
		...
		if (this.correctGuesses.length == 0){
			...
			for (var i=0; i<this.heroesLetters.length; i++){
				if (this.correctGuessesInOrder[i] != this.heroesLetters[i]){
					for (var j=0; j<this.correctGuesses.length; j++){
						if (this.correctGuesses[j] == this.heroesLetters[i]){
							this.correctGuessesInOrder[i] = this.heroesLetters[i];
						}
						else {
							this.correctGuessesInOrder[i] = "_";
						}
	...
	checkProgress: function(){
		...
		if (counter == this.heroesLetters.length){
			alert("You win");
			this.winCount++;
			this.generateWord();
		}
